...
<select v-model="selectedInvoiceNumber" class="form-select mt-3" aria-label="Select Invoice Number">
  <option disabled value="">Select Invoice Number</option>
  <option value="" selected>Show All</option>
  <option v-for="invoiceNumber in filteredInvoiceNumbers" :value="invoiceNumber" :key="invoiceNumber">
    {{ invoiceNumber }}
  </option>
</select>
...
const filteredInvoiceNumbers = computed(() => {
  ...
  const updateFilteredInvoiceNumbers = () => {
    if (!selectedCustomer.value) {
      filteredInvoiceNumbers.value = [];
      return;
    }
    ...
    filteredInvoiceNumbers.value = Array.from(invoiceNumbers);
  };
      ...
      <div>
        <select v-if="selectedTable && entries[selectedTable]" v-model="selectedEntry" class="form-select mt-3"
          ...
          <option v-for="entry in entries[selectedTable]" :key="entry.id" :value="entry">
           ...
           <select class="form-select mt-3" v-model="selectedInvoiceNumber" v-if="selectedCustomer"
        aria-label="Default select example">
        <option disabled value="">Select Invoice Number</option>
        <option value="" selected>Show All</option>
